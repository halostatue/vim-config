...
if neobundle#tap('neocomplete.vim') "{{{
  ...
  let neobundle#hooks.on_source = config.plugin('neocomplete')

  call neobundle#untap()
endif "}}}

if neobundle#tap('neosnippet.vim') "{{{
  let neobundle#hooks.on_source = config.plugin('neosnippet')
...
if neobundle#tap('vimshell.vim') "{{{
  ...
  let neobundle#hooks.on_source = config.plugin('vimshell')
...
if neobundle#tap('unite.vim') "{{{
  ...
  nnoremap <silent> [Window]<Space>  :<C-u>Unite -buffer-name=files file_rec:~/.vim/config<CR>
  ...
  if executable('wmctrl') && has('gui')
    nnoremap <silent> <C-w>  :<C-u>Unite -auto-resize window/gui<CR>
  endif

  let neobundle#hooks.on_source = config.plugin('unite')
...
if neobundle#tap('vim-smartchr') "{{{
  let g:neocomplete#enable_at_startup = 1
  let neobundle#hooks.on_source = config.plugin('smartchr')
...
if neobundle#tap('vim-ref') "{{{
  function! neobundle#hooks.on_source(bundle)
    ...
    autocmd HSAutoCmd FileType ref call s:ref_my_settings()
...
if neobundle#tap('vimfiler.vim') "{{{
  ...
  let neobundle#hooks.on_source = config.plugin('vimfiler')
...
if neobundle#tap('J6uil.vim') "{{{
  function! neobundle#hooks.on_source(bundle)
    ...
    autocmd HSAutoCmd FileType J6uil call s:j6uil_settings()
    autocmd HSAutoCmd FileType J6uil_say call s:j6uil_say_settings()
...
if neobundle#tap('qfreplace.vim') "{{{
  autocmd HSAutoCmd FileType qf nnoremap <buffer> r :<C-u>Qfreplace<CR>
...
if neobundle#tap('caw.vim') "{{{
  autocmd HSAutoCmd FileType * call s:init_caw()
  function! s:init_caw()
    if !&l:modifiable
